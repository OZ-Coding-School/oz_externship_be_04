...
class RecruitmentImageCreateSerializer(serializers.ModelSerializer):
    """
    구인공고 이미지 생성
    """
    image_url = serializers.URLField(source='img_url', help_text='이미지 URL')
...
class RecruitmentImageUpdateSerializer(serializers.ModelSerializer):
    """
    구인공고 이미지 수정
    """
...
class RecruitmentImageListField(serializers.ListField):
    ...
    def to_internal_value(self, data):
        ...
        if len(data) > 10:
            raise serializers.ValidationError("이미지는 최대 10개까지 등록 가능합니다.")
...
class RecruitmentImageResponseSerializer(serializers.ModelSerializer):
    """
    구인공고 이미지 응답
    """
