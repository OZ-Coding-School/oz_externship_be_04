...
class GroupScheduleSerializer(serializers.ModelSerializer):
    ...
    def validate_study_group(self, value):
        if not StudyGroup.objects.filter(study_group=value).exists():
    ...
    def validate(self, data):
        if data["session_date"] < datetime.today().date():
            raise serializers.ValidationError({"detail": "날자 설정이 잘못 되었습니다."})
